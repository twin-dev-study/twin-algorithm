...
# 수정사항
# 1차 2022.11.24.목 19:26 (1 print('방법n')을 추가해 출력값 구분 / 2 방법3에서 g[-1]+1이나 g[-1]이나 출력값이 같은 이슈 발생 / 3 방법4 해결못한부분 설명 어려웠던 거 이젠 설명 가능)
# 2차 2022.11.24.목 22:51 (방법 4-2 k+=1 위치 변경)
...
multiple31()
    ...
    ## 인사이트1
     # range(start, stop)으로 stop 부분은 '미만'이므로 o[-1]+1로 마지막 요소 값을 불러 와야 함
...
def multiple41():
    ...
    while j < 10:       # 1번째 while문 2~9단
        k = 1
        while k < 10:    # 2번째 while문 곱하는 숫자 1~9
            print(f'{j} * {k} = {j * k}')
            k += 1
        j += 1
multiple41()
...
multiple42()
    ...
    ## 인사이트2
     # 방법 4-2에서 k+=1 위치 변경하면 방법4-1처럼 k=1을 한 번만 작성해도 됨
